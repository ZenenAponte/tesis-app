...
export default function CausaList() {
  ...
  const [causa, setCausas] = useState([])
  ...
  const loadCausa = async () => {
    const response = await fetch('http://localhost:4000/causa')
  ...
  const handleDelete = async (id_causa) => {
    try {
      ...
      console.log(res)
      setCausas(causa.filter(causa => causa.id_causa !== id_causa));
  ...
  useEffect(() => {
    ...
    const filteredLugars = causa.filter((causa) =>
      causa.tipo.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    <Container style={{ width: '50%', margin: '0 auto', padding: '2rem 0' }} >
      ...
      <>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <Typography variant="h5" sx={{ flexGrow: 1 }}>Lista de Causas</Typography>
          <Button
            variant="contained"
            color="primary"
            onClick={() => navegate('/causa/new')}
          >
            Añadir
          </Button>
        ...
        {currentLugars.map((causa) => (
          ...
          >
            <CardContent style={{
              ...
              <div>
                <Typography>{causa.tipo}</Typography>
              </div>
              ...
              <div>
                <Button
                  variant="contained"
                  color="warning"
                  onClick={() => navegate(`/causa/${causa.id_causa}/edit`)}
                >
                  Editar
                </Button>
