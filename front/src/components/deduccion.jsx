...
const Deduccion = () => {
  ...
  const [currentPlace, setCurrentPlace] = useState({ id: '', name: '',descripcion: '' });

  const handleOpen = (place = { id: '', name: '',descripcion: ''}) => {
  ...
  const handleClose = () => {
    setOpen(false);
    setCurrentPlace({ id: '', name: '', descripcion: '' });
  };
  ...
  return (
    <Container sx={{ bgcolor: 'white', padding: 4, width: '50%', marginTop: 4 }}>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>{currentPlace.id ? 'Edit Place' : 'Añadir Tipo de Deducción'}</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
